...
#> cacheSolve(cm)			## rerun to see that cacheSolve will retrieve
                                        ## the cached inverse
...
cacheSolve <- function(x, ...) {
        ...
        ## if it's already cached, then it this function will stop and return
        ## the cached inverse.
        ## The inverse will be set to NULL automatically if the the matrix is
        ## changed (refer to x$set).
        xinv <- x$getinv()
        if(!is.null(xinv)) {
